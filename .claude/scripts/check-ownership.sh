...
case "$ROLE" in
  ...
  be2) echo "$FILE" | grep -qE "lib/signer/|lib/ledger/|lib/store/|app/api/${G}(documents|sign|webhooks|cron|evidence|ledger|dev|family-ack|estate|clm|me)/|supabase/" || deny "BE-2 소유 경로 밖: $FILE" ;;
