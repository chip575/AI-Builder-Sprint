...
function donationSession(...texts: string[]) {
  const s = getOrCreateSession();
  const first = addUtterance(s, texts[0] ?? "부산에 기부하고 싶어요");
...
describe("M-EXTRACT — 픽스처 4케이스", () => {
  it('"100만원이요" → amount 1,000,000 · HIGH · sourceSpan 있음', async () => {
    const s = donationSession("부산에 기부하고 싶어요", "100만원이요");
  ...
  it('"한 십만원쯤" → 100,000 · MEDIUM · 되묻기 대상', async () => {
    const s = donationSession("부산에 기부하고 싶어요", "한 십만원쯤 생각해요");
    const { body } = await extract(s.id);
  ...
  it("기부처(지역) 미언급 → missingRequired에 region", async () => {
    const s = getOrCreateSession();
    const u = addUtterance(s, "기부하고 싶어요");
    addProposal(s, "DONATION_NOW", "EXPRESS", u.id);
    addUtterance(s, "100만원이요");
    const { body } = await extract(s.id);
  ...
  it("모든 fact는 confirmed=false로 시작한다 (P1)", async () => {
    const s = donationSession("부산에 100만원 기부하고 싶어요");
    const { body } = await extract(s.id);
