...
| 갱신 도래가 화면에 없던 것 | `/org`가 `/api/admin/summary`를 부르고 목록을 렌더한다. 빈 목록도 "없습니다"로 말한다 |

**심은 것**: 갱신 도래 4건 — `node scripts/seed-obligations.mjs 2026-08-01`.
이 약속은 정기후원 문서가 **완료될 때 12개월 뒤로** 생성되므로(`webhooks/outbox.ts:97`)
기다리지 않으면 채울 방법이 없다. 화면은 정상인데 데이터가 없는 상태였고,
그건 정적 분석으로도 HTTP 200으로도 안 드러난다 — "고칠 것"이 아니라 "심을 것"인 이유다.

**남은 것**: `/ledger` 진입로(§3 — 제품 결함으로 존치) ·
`/doc` 새로고침 상태 유실 · 가지 제안 SSE 미수신 · 로그인 실통과 확인(사람).
