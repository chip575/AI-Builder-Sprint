...
**훅의 알려진 한계**: 훅은 `Edit`·`Write`·`MultiEdit`만 검사한다.
**Bash를 거친 파일 수정(`sed`·python 등)은 잡지 않는다.** 위협 모델이 *악의*가 아니라
*실수*이기 때문이다 — 워커가 무심코 계약을 고치는 것은 막지만, `sed`로 계약을 고치는 일은
무심코 일어나지 않는다. Bash 명령 문자열에서 경로를 뽑는 검사는 파이프·변수·상대경로에서
정상 명령을 막는 오탐이 더 크다.

**그래서 최종 방어선은 훅이 아니라 `git diff`와 병합 전 사람 리뷰다.**
보호 경로(`lib/contracts`·`lib/rules`·validity-gate·RLS·서명 URL)의 변경은 훅이
막았는지와 무관하게 **병합 전에 사람이 diff로 확인한다.** 훅은 실수를 줄이는 장치이지
통과했다는 사실이 승인을 뜻하지 않는다.
